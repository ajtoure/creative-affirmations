...
const randomizeNumber = array => {
    let newNumber = array[Math.floor(Math.random() * array.length)];
    return newNumber;
}

let newQuote = randomizeQuote(affData.quotes);
let newNumb = randomizeNumber(affData.angelNumbers);
...
const createOutput = (angelNumb, affirmation) => {
    return {
...
let finalOutput = createOutput(newNumb, newQuote);

//console.log(finalOutput.affirmation);

const logMsg = (finalMsg) => {
    return `
    Your angel number is: ${finalMsg.angelNumber}
    It means that ${finalMsg.meaning}
    Repeat this affirmation today: ${finalMsg.affirmation}`;
}

console.log(logMsg(finalOutput));

//console.log(logMessage(finalOutput));
//THURS 
//1. hook up to git
